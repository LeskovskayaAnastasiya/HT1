...
public class Person {
    ...
    public boolean validateFMLNamePart(String fml_name_part, boolean empty_allowed)
    {
        ...
        if (empty_allowed)
        {
            p = Pattern.compile("[\\w-|а-яА-я]{0,150}", Pattern.UNICODE_CHARACTER_CLASS);
        }
        else
        {
            p = Pattern.compile("[\\w-|а-яА-я]{1,150}", Pattern.UNICODE_CHARACTER_CLASS);

        }
        Matcher matcher = p.matcher(fml_name_part.trim());
        return matcher.matches();
    }
    public void deletePhone(String  iDphone)
    {
        phones.remove(iDphone);
    }
    // ++++++++++++++++++++++++++++++++++++++
    // Геттеры и сеттеры
    public String getId()
    ...
    public String getIdPhone(String phoneNumber) {
        for (String iDphone : phones.keySet()){
            String value = phones.get(iDphone);
            if (value == phoneNumber){
                idPhoneNumber =  iDphone ;
            }
        }
