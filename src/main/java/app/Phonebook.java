...
public class Phonebook {
    ...
    public void addPhoneToDb(String number, String ownerId){
        ResultSet db_result;
        String query = "INSERT INTO `phone` ( `owner`, `number`) VALUES ('" + ownerId +"', '" + number + "')";
        db.changeDBData(query);
    }

    public String getPhoneFromDb(String id){
        String number = null;
        ResultSet db_number = DBWorker.getInstance().getDBData("SELECT number FROM phonebook.phone where id = " + id);
        try
        {
            // Если у человека нет телефонов, ResultSet будет == null.
            while (db_number.next())
            {
                number =  db_number.getString("number");
            }
    ...
    public void updatePhoneFromDb(String idPhone,String idPerson, String number){
        String query = "";
        query = "REPLACE into phonebook.phone (`id`, `owner`, `number`) values (" + idPhone +" , " + idPerson + " , ' "+ number + " ' );";
        db.updataPhoneNumber(query);
    }
